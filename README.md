...
The Schwab runtime can execute all seven live `us_equity` profiles from `UsEquityStrategies`.
...
| `tech_communication_pullback_enhancement` | Tech/Communication Pullback Enhancement | Yes | Yes | No | No | `us_equity` | enabled feature-snapshot tech branch |
...
| `STRATEGY_PROFILE` | Strategy profile selector (default: `tqqq_growth_income`; current enabled values also include `dynamic_mega_leveraged_pullback`, `global_etf_rotation`, `mega_cap_leader_rotation_dynamic_top20`, `russell_1000_multi_factor_defensive`, `soxl_soxx_trend_income`, `tech_communication_pullback_enhancement`) |
...
- **Repository Variables**
  ...
  - Optional: `STRATEGY_PROFILE` (recommended: `tqqq_growth_income`; other enabled values: `dynamic_mega_leveraged_pullback`, `global_etf_rotation`, `mega_cap_leader_rotation_dynamic_top20`, `russell_1000_multi_factor_defensive`, `soxl_soxx_trend_income`, `tech_communication_pullback_enhancement`)
...
- `INCOME_THRESHOLD_USD` and `QQQI_INCOME_RATIO` are optional in env sync. Leave them unset to inherit the `UsEquityStrategies` profile defaults; the current `tqqq_growth_income` live default is the no-income QQQ/TQQQ dual-drive mode.
...
Schwab runtime 现在可以直接执行 `UsEquityStrategies` 里的全部 7 条 live `us_equity` 策略：`dynamic_mega_leveraged_pullback`、`global_etf_rotation`、`mega_cap_leader_rotation_dynamic_top20`、`russell_1000_multi_factor_defensive`、`tqqq_growth_income`、`soxl_soxx_trend_income` 和 `tech_communication_pullback_enhancement`。
...
| `STRATEGY_PROFILE` | 策略档位选择（默认: `tqqq_growth_income`；当前已启用值还包括 `dynamic_mega_leveraged_pullback`、`global_etf_rotation`、`mega_cap_leader_rotation_dynamic_top20`、`russell_1000_multi_factor_defensive`、`soxl_soxx_trend_income` 和 `tech_communication_pullback_enhancement`） |
...
- **仓库级 Variables**
  ...
  - 可选：`STRATEGY_PROFILE`（建议设为 `tqqq_growth_income`；其他已启用值还有 `dynamic_mega_leveraged_pullback`、`global_etf_rotation`、`mega_cap_leader_rotation_dynamic_top20`、`russell_1000_multi_factor_defensive`、`soxl_soxx_trend_income` 和 `tech_communication_pullback_enhancement`）
...
- `STRATEGY_PROFILE` 现在由平台能力矩阵和从 `runtime_enabled` 策略元数据派生的 rollout allowlist 一起决定。当前 `eligible` 和 `enabled` 都包含全部 7 条 live `us_equity` 策略：`dynamic_mega_leveraged_pullback`、`global_etf_rotation`、`mega_cap_leader_rotation_dynamic_top20`、`russell_1000_multi_factor_defensive`、`tqqq_growth_income`、`soxl_soxx_trend_income` 和 `tech_communication_pullback_enhancement`。
- 当前策略域是 `us_equity`，本地策略注册表只用于域和 profile 校验。
- `INCOME_THRESHOLD_USD` 和 `QQQI_INCOME_RATIO` 在 env-sync 里是可选项。不填时会继承 `UsEquityStrategies` 的 profile 默认值；当前 `tqqq_growth_income` 实盘默认是不带收入层的 QQQ/TQQQ 双轮模式。
